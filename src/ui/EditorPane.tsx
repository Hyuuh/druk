...
export function EditorPane(props: EditorPaneProps) {
  ...
  const inlineNotes = createMemo(() => {
    ...
    const el = editorEl()
    if (!el || !host) return []
    // Both annotations share the geometry: one map of line → what to draw, built
    // with the review marks first so a diagnostic on the same line wins it.
    const wanted = new Map<number, { text: string; color: string }>()
    if (props.reviewText) {
      for (const [line, mark] of props.reviews) {
        wanted.set(line, {
    ...
    for (const [line, note] of wanted) {
      ...
      notes.push({
        ...
        left,
        text: cut(note.text.replaceAll(/\s+/g, ' '), room),
        color: note.color,
      })
    }
    return notes
  ...
  const syncCursor = () => {
    ...
    const span = editor.hasSelection() ? editRange(editor.plainText) : null
