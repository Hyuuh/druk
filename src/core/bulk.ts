...
export async function removeAll(
  ...
  for (const target of targets) {
    ...
    for (const unit of units) {
      ...
      onProgress({ done, total, current: basename(unit) })
      await yieldToLoop()
    }
...
export async function copyAll(
  ...
  for (const [index, target] of targets.entries()) {
    ...
    try {
      ...
      await fs.promises.cp(target, to, { recursive: true, force: false, errorOnExist: true })
      moved.push({ from: target, to })
      done++
    } catch {
      failed.push(basename(target))
    }
    onProgress({ done, total: targets.length, current: basename(target) })
...
export async function moveAll(
  ...
  for (const [index, target] of targets.entries()) {
    ...
    try {
      ...
      try {
        await fs.promises.rename(target, to)
      } catch (error) {
