 ...
 * whole trade — a plugin adds themes, icon themes and language servers, and
 * nothing else, which is what these three lists are for.
 *
...
export function parseManifest(
  ...
  const servers = list(raw.languageServers)
    ...
    .filter(entry => entry !== null)
  if (themes.length === 0 && icons.length === 0 && servers.length === 0) {
